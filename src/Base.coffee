		...
		find: (id, cb) ->
			...
			if window.Offline
				@dbmodel.load id, (dbobj) ->
					...
					obj = new window[self.getBtype()] data
		...
		prefetch: (cb) ->
			...
			fetchlist = (keyval, cb2) ->
				...
				if val.embedded
					...
					q2 = db2.all().filter(self.key, '=', self.id).list (results) ->
						objs = []
						for obj in results
		...
		save: (edited, cb) ->
			...
			if @_parent
				...
				parent.prefetch () ->
					...
					list = parent.get self._list
					parent.set self._list, []
					for obj in list
						parent.push self._list, if obj.id == self.id then self else obj
					self.saveLocal()
